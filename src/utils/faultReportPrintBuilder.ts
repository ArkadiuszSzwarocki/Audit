/**
 * Generates a printable HTML document for a Fault report
 * and opens the browser print dialog.
 */

export interface FaultPrintData {
  id: string;
  title: string;
  description: string;
  severity: string;
...
export function printFaultReport(data: FaultPrintData, baseUrl: string = typeof window !== 'undefined' ? window.location.origin : ''): void {
...
<head>
  <meta charset="UTF-8"/>
  <title>Zgłoszenie Usterki — ${data.title}</title>
  <style>
    @media print {
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header { background: #2563eb; padding: 24px 28px; color: #fff; }
    .header .app-label { font-size: 10px; font-weight: 700; color: #dbeafe; text-transform: uppercase; letter-spacing: 0.12em; margin-bottom: 4px; }
    .header h1 { font-size: 20px; font-weight: 800; margin-bottom: 4px; }
    .header .date { font-size: 12px; color: #bfdbfe; }
    .body { padding: 24px 28px; }
    .status-pill { display: inline-block; padding: 4px 12px; font-size: 11px; font-weight: 800; border-radius: 8px; border: 1px solid #bfdbfe; background: #eff6ff; color: #1e40af; margin-bottom: 16px; margin-right: 8px; }
...
<body>
  <div class="card">
    <div class="header">
      <div class="app-label">System AuditApp — Raport Usterki / Awarii</div>
      <h1>🔧 ${data.title}</h1>
    ...
    <div class="body">
      ...
      <div class="section">
        <div class="section-label">Opis Usterki</div>
      ...
      <div class="grid">
        ...
        ${data.assignedToName ? `<div class="grid-item"><div class="label">Przypisano do</div><div class="value">🔧 ${data.assignedToName}</div></div>` : ''}
        ...
        ${data.dueDate ? `<div class="grid-item"><div class="label">Termin Realizacji</div><div class="value">📅 ${new Date(data.dueDate).toLocaleDateString('pl-PL')}</div></div>` : ''}
      </div>

      ${data.operatorComment ? `
      <div class="section action">
        <div class="section-label">✅ Podjęte Działania / Komentarz</div>
        <div class="section-text">${data.operatorComment}</div>
        ${data.fixedBy ? `<div style="font-size:11px;font-weight:700;color:#047857;margin-top:6px;">Naprawił: ${data.fixedBy} (${data.fixedAt ? new Date(data.fixedAt).toLocaleString('pl-PL') : ''})</div>` : ''}
      </div>` : ''}
