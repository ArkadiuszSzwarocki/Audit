/**
 * Generates a printable HTML document for a BHP hazard report
 * and opens the browser print dialog.
 */

export interface BhpPrintData {
  id: string;
  title: string;
  description: string;
  category: string;
...
const CATEGORY_MAP: Record<string, string> = {
  NEAR_MISS: '⚠️ Zdarzenie Potencjalnie Wypadkowe (Near Miss)',
  UNSAFE_COND: '🛠️ Niebezpieczne Warunki Pracy',
  UNSAFE_BEHAVIOR: '🚷 Niebezpieczne Zachowanie',
  FIRE_HAZARD: '🔥 Zagrożenie Pożarowe',
  PPE: '🥽 Brak lub Uszkodzenie ŚOI',
};

const SEVERITY_MAP: Record<string, string> = {
  CRITICAL: '🔴 Wysokie Ryzyko (Krytyczne)',
...
const STATUS_MAP: Record<string, string> = {
  OPEN: '🔓 Zgłoszone',
  IN_PROGRESS: '⚙️ W eliminacji',
  RESOLVED: '🛡️ Wyeliminowane',
  CLOSED: '🔒 Zamknięte',
};

export function printBhpReport(data: BhpPrintData, baseUrl: string): void {
...
<head>
  <meta charset="UTF-8"/>
  <title>Zgłoszenie BHP — ${data.title}</title>
  <style>
    @media print {
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header { background: #ea580c; padding: 24px 28px; color: #fff; }
    .header .app-label { font-size: 10px; font-weight: 700; color: #ffedd5; text-transform: uppercase; letter-spacing: 0.12em; margin-bottom: 4px; }
    .header h1 { font-size: 20px; font-weight: 800; margin-bottom: 4px; }
    .header .date { font-size: 12px; color: #fed7aa; }
    .body { padding: 24px 28px; }
    .status-pill { display: inline-block; padding: 4px 12px; font-size: 11px; font-weight: 800; border-radius: 8px; border: 1px solid #fdba74; background: #fff7ed; color: #c2410c; margin-bottom: 16px; margin-right: 8px; }
...
<body>
  <div class="card">
    <div class="header">
      <div class="app-label">System AuditApp — Raport Zdarzenia BHP</div>
      <h1>🛡️ ${data.title}</h1>
    ...
    <div class="body">
      ...
      <div class="section">
        <div class="section-label">Opis Zagrożenia BHP</div>
      ...
      <div class="grid">
        ...
        ${data.assignedToName ? `<div class="grid-item"><div class="label">Inspektor BHP / Przypisany</div><div class="value">🛡️ ${data.assignedToName}</div></div>` : ''}
        ...
        ${data.dueDate ? `<div class="grid-item"><div class="label">Termin Eliminacji</div><div class="value">📅 ${new Date(data.dueDate).toLocaleDateString('pl-PL')}</div></div>` : ''}
      </div>

      ${data.actionTaken ? `
      <div class="section action">
        <div class="section-label">🛡️ Podjęte Działania Korygujące</div>
        <div class="section-text">${data.actionTaken}</div>
        ${data.fixedBy ? `<div style="font-size:11px;font-weight:700;color:#047857;margin-top:6px;">Wyeliminował: ${data.fixedBy} (${data.fixedAt ? new Date(data.fixedAt).toLocaleString('pl-PL') : ''})</div>` : ''}
      </div>` : ''}
