...
export function KaizenDrawer({ kaizen, onClose, onViewHistory, isAdmin, onDelete }: KaizenDrawerProps) {
  ...
  const { isKaizenCommittee, user } = useAuth();
  const [modalImage, setModalImage] = useState<string | null>(null);
  const [isEmailModalOpen, setIsEmailModalOpen] = useState(false);

  const canEvaluate = isAdmin || isKaizenCommittee || (user?.role && ['KOMISJA KAIZEN', 'KOMISJA_KAIZEN', 'KAIZEN_COMMITTEE'].includes(user.role.toUpperCase()));
